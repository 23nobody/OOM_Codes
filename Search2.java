...
class Search2 {
	static void dijkstra(String source,ArrayList<Landmark> dest, ArrayList<Vertex> adjlist, HashMap<String, Integer> map,int v) {
// 		if(dest==source) {
...
// 		}
		...
		for(int i=0;i<dest.size();i++){
		    Landmark k = dest.get(i);
		    if(map.containsKey(k.from)) {
		int g = map.get(k.from);
	    if(cost[g]<= min ){  
		min = cost[g];
		index = g;
		}
		...
		print(cost,index,parentRoad,map);
    ...
    public static void main(String[] args){
		...
		for(int j=0;j<l;j++) {
			...
			case 3:
				...
				y =  new Washroom(id,name,location,from,to,costPerUse,cleanlinessLevel,type);
				break;
		...
		for(int z = 0 ; z<q;z++) {
			...
			ArrayList<Landmark> list2 = new ArrayList<>();
			...
			for(int b = 0 ; b<landmarks.size();b++){
				...
				dijkstra(from,list2,adjList,map,v);
...
class Washroom extends Landmark{
	...
	Washroom(String id, String name, String location,String from,String to, String costPerUse,String cleanlinessLevel,String type) {
		super(id, name, location, from, to);
